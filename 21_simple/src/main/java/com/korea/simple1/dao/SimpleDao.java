...
public class SimpleDao {
	...
	// Connection Pool 처리는 DataSource 클래스가 한다
	// 필요한 설정은 톰캣 내부의 context.xml에 있다.
	// <Resource name="jdbc/oracle">을 처리하는 Context 클래스가 필요
	// name에 의한 접근 : JNDI
	...
	// singleton
	private SimpleDao() {
		// context.xml에 있는 설정을 읽어와서 dataSource를 만든다.
		try {
			Context context = new InitialContext();
			dataSource = (DataSource)context.lookup("java:comp/env/jdbc/oracle");  
			// 톰캣이다: java:comp/env/ 나머지는 <Resource>의 name 속성
	...
	public static SimpleDao getInsetance() {
		return simpleDao;
	}
	...
	public SimpleDto simpleView(int no) {
		...
		try {
			...
			if (rs.next()) {
				...
				String content = rs.getString("CONTENT");
				Date regDate = rs.getDate("REGDATE");
				simpleDto = new SimpleDto(no, title, writer, content, regDate);
			}
